  ...
  return (
    <ClerkProvider>
      <html lang="en" className={`${playfair.variable} ${dmSans.variable}`}>
        <body className="bg-bone text-espresso font-sans min-h-screen">
          ...
          {/* Navigation */}
          <nav className="border-b border-(--taupe)/30 bg-(--bone)/90 backdrop-blur-sm sticky top-0 z-40">
            <div className="max-w-5xl mx-auto px-6 lg:px-8">
              <div className="flex items-center justify-between h-20">
                {/* Logo */}
                <Link
                  href="/"
                  className="font-serif text-2xl tracking-wide text-coffee hover:text-espresso transition-colors duration-300"
                >
                  Espress
                </Link>
                ...
                <div className="flex items-center gap-6 md:gap-10">
                  ...
                  <SignedOut>
                    <SignInButton mode="modal">
                      <button className="text-sm tracking-widest uppercase text-mocha hover:text-espresso transition-colors duration-300 py-2">
                        Sign In
                      </button>
                    </SignInButton>
                  </SignedOut>
          ...
          {/* Footer */}
          <footer className="border-t border-(--taupe)/20 mt-auto">
            <div className="max-w-5xl mx-auto px-6 lg:px-8 py-8">
              <p className="text-center text-sm text-latte tracking-wide">
                Savor every sip
              </p>
            </div>
          </footer>
...
function NavLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      className="relative text-sm tracking-widest uppercase text-mocha hover:text-espresso transition-colors duration-300 py-2 group"
    >
      {children}
