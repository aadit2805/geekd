...
router.post('/', async (req, res) => {
  try {
    ...
    // Check if already in wishlist or already visited
    if (place_id) {
      const [existing, visited] = await Promise.all([
        pool.query('SELECT id FROM wishlist WHERE user_id = $1 AND place_id = $2', [userId, place_id]),
        pool.query('SELECT id FROM cafes WHERE user_id = $1 AND place_id = $2', [userId, place_id]),
      ]);
...
router.post('/:id/visit', async (req, res) => {
  try {
    ...
    // Use a transaction so the cafe insert and wishlist delete are atomic
